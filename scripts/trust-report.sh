#!/usr/bin/env bash
# ────────────────────────────────────────────────────────────────────────────
# VENDORED trust-report (shared build-hygiene checker)
# SYNCED:        2026-06-14T18:34:09Z
# MASTER SHA256: 65a28ac67f80b1088c982e0ccc11cf08ed99990f0d4a4289d535677da16cf3a6
...
: >"$NET_OUT"
# JS/TS · Rust · Python · Dart/Flutter (HttpClient, package:http, dio, Uri.http(s),
# WebSocket) and RevenueCat (Purchases.) so disclosed SDK calls surface for review.
PATTERN='fetch\(|XMLHttpRequest|axios|http\.get|http\.post|https?://[a-zA-Z]|reqwest|isahc|ureq|hyper::|tonic|HttpClient|package:http|Dio\(|dio\.|Uri\.parse|Uri\.https?\(|WebSocket\.connect|Purchases\.'
for sub in src crates packages apps webapp frontend www tests lib test; do
  [ -d "$ROOT/$sub" ] || continue
  grep -rEn \
    --include='*.ts' --include='*.tsx' --include='*.js' --include='*.mjs' --include='*.jsx' \
    --include='*.rs' --include='*.py' --include='*.dart' \
